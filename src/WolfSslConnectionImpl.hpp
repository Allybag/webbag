...
// Internal header - shared implementation details for WolfSSL connections
...
// Base implementation shared by client and server
class WolfSslConnectionBase::Impl
{
public:
    using TimestampNanos = WolfSslConnectionBase::TimestampNanos;
    ...
    std::size_t write(const void* data, std::size_t size)
    {
        ...
        if (result != static_cast<int>(size))
        {
            ...
            while (error == WOLFSSL_ERROR_WANT_WRITE && result == cErrorResult)
            {
...
protected:
    ...
    {
        auto* impl = static_cast<WolfSslConnectionBase::Impl*>(context);
